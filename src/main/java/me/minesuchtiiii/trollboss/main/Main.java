package me.minesuchtiiii.trollboss.main;

import me.minesuchtiiii.trollboss.commands.*;
import me.minesuchtiiii.trollboss.listeners.*;
...
public class Main extends JavaPlugin {
    ...
    public void kickSchedu(final Player p) {
        ...
        Bukkit.getScheduler().runTaskLater(this, () -> {
            ...
            for (Player all : playersToKick) {
                Main.this.res.add(all.getUniqueId());
    ...
    public void removeFromRunIt(Player p) {
        ...
        if (this.runIt.contains(p.getUniqueId())) {
            ...
            Bukkit.getScheduler().scheduleSyncDelayedTask(this, () -> Main.this.runIt.remove(p.getUniqueId()), 80L);
    ...
    private void addInventoryTextItem(Player p, int inventorySlot, String text, int amount) {
        ...
        final Random x = new Random();
        final int a = x.nextInt(Main.this.color.size());
    ...
    private void check4movement(Player p) {
        ...
        final int checki = Bukkit.getScheduler().scheduleSyncRepeatingTask(this, () -> {
            ...
            if (Main.this.rfloc.containsKey(p.getName())) {
                if (Main.this.rfloc.get(p.getName()).equals(p.getLocation())) {

                    p.sendMessage("§7[§4INFO§7] §cAll you had to do was to damn move CJ!");

                    Main.this.rf.replace(p.getName(), false);
                    Main.this.deadMessage.add(p.getUniqueId());
                    Main.this.cancelTask(tasks2, p);

                    final String trollername = Main.this.rfmsg.get(p.getName());
                    ...
                    if (troller != null) {
                        ...
                        Main.this.rfmsg.remove(p.getName());
